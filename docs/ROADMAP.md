...
- [~] `notification-service` (Go, `go mod init` subdir of `apps/go`): WebSocket/SSE, Kafka consumer, push (FCM/APNs)
  - [x] Slice 1: Kafka consumer → persisted inbox + live SSE. Consumes `sssm.post-events` (kafka-go, `FetchMessage`+`CommitMessages` at-least-once, routes by the `event-type` header) and turns engagements (like/repost/bookmark) and replies into per-recipient notifications — resolving the recipient by reading post-service's `sssm.posts` (read-only, the timeline-service single-RDS pattern), skipping self-actions and remove/view events. Notifications persist to infra-owned `sssm.notifications` (idempotent dedupe of redelivery via a `(recipient,actor,kind,post)` unique key) and `sssm.device_tokens`; `PostDeleted` purges referenced rows. HTTP API (gateway-trusted `X-Auth-Subject`): `GET /api/v1/notifications` (keyset-paged), `/unread-count`, `POST /read`, **SSE** `GET /stream` (in-process Hub, best-effort, persisted-first), and device register/unregister. Push is a stubbed `Pusher` interface (logs; real FCM/APNs deferred like media-service's Kafka). First Go consumer of the proto contract → shared pure-Go `common/eventv1` protowire decoder (no protoc in the local toolchain). Registered in `sssm-services` Helm values (port 8084) + `flyway-notification` runner/`make migrate`. `go test ./...` green — 29 unit tests (6 `eventv1` wire round-trips + 23 in-service: cursor, Hub fan-out, event-handling + pagination + device validation, consumer routing) with fakes.
  - [~] Slice 2: real FCM/APNs pusher; Redis pub/sub so any replica reaches any client (replace in-process Hub); follow/mention notification kinds (needs user-service follow events)
    - [x] Redis pub/sub SSE fan-out: replaced Slice 1's in-process `Hub` with a `RedisHub` that keeps a per-replica registry of connected streams and bridges replicas over one Redis channel per recipient (`notifications:{userId}`). A live stream's first connection on a replica `SUBSCRIBE`s the recipient's channel and the last to leave `UNSUBSCRIBE`s it (so a replica only listens for users it serves); `Publish` `PUBLISH`es the JSON notification to that channel, and every replica with the recipient connected delivers it to its local streams — so a notification now reaches the recipient regardless of which replica their SSE connection landed on. The recipient is routed by channel name, never the payload. Delivery stays best-effort/persisted-first (Redis errors logged + swallowed; inbox is the backstop). New required `REDIS_URL` (registered in `sssm-services` Helm values); the cross-replica transport is an interface (`PubSub`) so the fan-out is unit-tested with a fake. `go test ./...` green — 31 tests (added `localRegistry` first/last + buffer-drop and `RedisHub` subscribe-management/publish-broadcast/cross-replica-deliver/misroute-guard).
    - [ ] real FCM/APNs pusher (read `device_tokens` → provider SDK; replace `logPusher` stub)
    - [ ] follow/mention notification kinds (needs user-service follow events: new proto event + user-service outbox + consumer)
- [x] `engagement-service` (Go, `go mod init` subdir of `apps/go`): like/view/repost counters in Redis, periodic flush to PG. Consumes `sssm.post-events` (reuses `common/eventv1` + the kafka-go consumer pattern) and applies `PostEngagement` to per-post Redis counters (`engagement:{postId}` hash, atomic `HINCRBY`: like/repost ±1, view +1; bookmark/unspecified ignored — post-service owns bookmark display counts), marking each touched post in a dirty set; `PostDeleted` drops the counters. A periodic flusher drains the dirty set (`SPOP` batch, requeue-on-failure) and upserts the durable snapshot in infra-owned `sssm.post_metrics` (a final flush runs on shutdown). Read API `GET /api/v1/metrics/posts/{id}` (gateway-trusted; dedicated `/api/v1/metrics` prefix to avoid the post-service ingress collision) serves live Redis counters, falling back to the PG snapshot on miss/outage, zeros when never engaged. These high-throughput counters (incl. **views**, which post-service does not persist) are distinct from post-service's transactional display counts and feed Phase 3 ranking. Registered in `sssm-services` Helm values (port 8085) + `flyway-engagement` runner/`make migrate`. `go test ./...` green — 13 unit tests (delta mapping, read fallback, flush/requeue, consumer routing) with fakes.
- [x] `search-service` (Go, `go mod init` subdir of `apps/go`) + OpenSearch 2.x (UPGRADE off ES 6.4.2); indexing via Kafka — a derived read model written ONLY from the event stream: consumes `sssm.post-events` (reuses `common/eventv1` + the kafka-go `FetchMessage`+`CommitMessages` at-least-once pattern, routes by the `event-type` header), indexing `PostCreated` as an OpenSearch document keyed by post id (`PUT /_doc/{id}` — idempotent under redelivery) and removing it on `PostDeleted` (404 = no-op); ignores `PostEngagement` (likes don't change searchable content). The index can always be rebuilt by replaying the topic, so it owns no SQL schema (no migration/Flyway runner). OpenSearch is driven over its REST API with the standard library only (dependency-light, matching the fleet) — `EnsureIndex` creates an explicit mapping (text full-text field; ids/keywords; `created_at` date). Read API `GET /api/v1/search/posts?q=&author=&limit=&cursor=` (gateway-trusted `X-Auth-Subject`): a bool query (match on `text`, or `match_all` when `q` empty, plus optional `author_id` term filter) sorted by relevance then recency, with opaque base64 offset-cursor pagination (`NextCursor` only when more remain). Registered in `sssm-services` Helm values (port 8086) + multi-stage distroless `Dockerfile` (no migration needed). `go test ./...` green — 20 unit tests (index PUT/DELETE/search-DSL + match_all + 404/500 handling, consumer routing + poison-message drop, cursor round-trip/clamp/pagination) with fakes.
- [x] `messaging-service` (Go, `go mod init` subdir of `apps/go`): 1:1 DMs over WebSocket — clients open one socket (`GET /api/v1/messages/stream`, gateway-trusted `X-Auth-Subject`) to send DMs (`{recipientId, body}` frames) and receive them live as `{type:"message"|"error"}` frames; a REST surface serves the keyset-paged conversation list (`GET /api/v1/messages/conversations`) and message history (`GET /api/v1/messages/conversations/{id}`) plus a non-WS send fallback (`POST /api/v1/messages`). Each message is persisted FIRST (infra-owned `sssm.dm_messages` + `sssm.dm_conversations`, the durable record) then fanned out best-effort over Redis pub/sub to BOTH participants' per-user channels (`dm:{userId}`) — so the recipient receives it on whatever replica their socket landed on, and the sender's other devices echo it for multi-device sync. A conversation is identified by its canonically-ordered participant pair (one row per pair, either direction); message-history reads are authorized by a participant join (a non-participant simply gets an empty page). WebSocket via dependency-free `github.com/coder/websocket`, kept behind a `WSConn` interface so the read/write pumps are unit-tested with a fake; the cross-replica `PubSub` and the `RedisHub` mirror notification-service's proven Slice 2 fan-out. Registered in `sssm-services` Helm values (port 8087) + `flyway-messaging` runner/`make migrate` + multi-stage distroless `Dockerfile`. No Kafka consumer (DMs originate from the API, not domain events). `go test ./...` green — 22 unit tests (cursor round-trip/reject, registry first/last + buffer-drop, RedisHub subscribe-management/publish/cross-replica-deliver/misroute-guard, service send-validation/persist/dual-fan-out/conversation-reuse/participant-authz/pagination, session read-forwards-send/write-delivers/error-frames) with fakes.
